                    ...
                    <form action="/bot/jobs/{{$job->id}}" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input type="text" name="max_value" value="{{old('max_value',$job->max_value)}}" placeholder="np. 100" class="form-control ">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                <input type="text" name="min_profit" value="{{old('min_profit',$job->min_profit)}}" placeholder="np. 2" class="form-control ">
                                <div class="input-group-append">
                                    <div class="input-group-text">PLN</div>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="market_id" value="{{old('market_id',$job->market_id)}}" class="select2 form-control" style="width: 100%;">
                                <option value="{{null}}"></option>
                                @foreach($markets as $market)
                                    <option value="{{$market->id}}" @if(old('market_id',$job->market_id) == $market->id) selected @endif >{{$market->market_code}}</option>
                                @endforeach
                            </select>
                            @error('market_id')
                            <span class="text-danger">{{$message}}</span>
